...
### Task & Project editors

When the active note's frontmatter declares `type: task` or `type: project` (per ADR 0115), [`EditorContentLayout`](../src/components/editor-content/EditorContentLayout.tsx) wraps the editor body with `TaskEditor` / `ProjectEditor` from [`src/components/tasks/`](../src/components/tasks/). The wrapper renders a row of property cells above the existing BlockNote / raw editor body — status, priority, due/start/completed dates, estimate, labels, project, assignees, blocked-by — without touching the body editor itself. Property writes route through the same `onUpdateFrontmatter` (→ `runFrontmatterAndApply` → `save_note_content`) pipeline that the Inspector uses, so there is no parallel save path. Typed accessors live in [`src/lib/tasks/`](../src/lib/tasks/) (`TaskView`, `ProjectView`, `DateOrDateTime`) and mirror the Rust [`TaskView` / `ProjectView`](../src-tauri/src/vault/task.rs) shape 1:1. Creating a task or project from the command palette (`New Task` / `New Project`) reuses the existing immediate-create flow with `type='task'` / `type='project'` — see [`src/hooks/commands/noteCommands.ts`](../src/hooks/commands/noteCommands.ts).

### View engine: field namespaces and multi-view files

Saved views in `views/*.yml` resolve filter / group / column fields through a small namespace resolver in [`src-tauri/src/vault/view_fields.rs`](../src-tauri/src/vault/view_fields.rs). `note.<name>` reads the frontmatter property (or one of the structural aliases `type`/`status`/`title`/`body`), `file.<name>` reads filesystem and entry metadata (locked v1 names: `name`, `path`, `folder`, `ext`, `size`, `ctime`, `mtime`, `tags`), and `formula.<name>` is reserved for v2 and always resolves to empty. Bare names (e.g. `priority`) keep working — the resolver treats them as `note.<name>`. The same resolver handles boolean fields (`archived`, `favorite`) under both bare and `note.` forms.

A single view file can hold one definition (the existing shape) or a top-level `views:` array of definitions. `scan_views` flattens multi-view files into individual `ViewFile` entries with synthetic filenames of the form `{base}#{index}`, and each `ViewFile` carries a `shape: Single | Multi` marker. `save_view` and `delete_view` reject synthetic filenames in v1 — programmatic edits to multi-view files are deferred until a UI caller needs them. The new `ViewDefinition` fields `display` (`list`/`table`/`board`/`timeline`/`cards`), `groupBy` (`{ property, direction }`), and `columns` are all optional with `skip_serializing_if` so existing view files round-trip byte-stable.

### Board view

When a saved view declares `display: board` or `display: table`, [`App.tsx`](../src/App.tsx) replaces the standard sidebar/note-list/editor layout with an `app__view-canvas` slot that absorbs all width right of the sidebar via `flex: 1`. The editor on the right shrinks to a fixed 420px detail column (`.app:has(.app__view-canvas) .app__editor` override; resizable 320–640px) so the canvas can be the focus.

For `display: board`, the canvas renders [`TaskBoard`](../src/components/tasks/TaskBoard.tsx). Columns come from [`deriveBoardColumns`](../src/lib/tasks/boardColumns.ts): if the visible tasks reference a project via wikilink and that project declares `statuses`, those statuses become the column order verbatim; otherwise columns are the distinct values of `groupBy.property` (default `status`) plus an `(unset)` bucket when any entry lacks a value. Each card is a [`TaskCard`](../src/components/tasks/TaskCard.tsx) shared primitive — title, priority chip, due-date badge, assignee count — degrading to title-only for non-task entries. Drag-drop uses `@dnd-kit/core` `useDraggable` / `useDroppable`; on drop, `planBoardDrop` resolves the new value and the existing `onUpdateFrontmatter` → `runFrontmatterAndApply` → `save_note_content` pipeline writes the new status (or `null` for the unset column) to the task's frontmatter. A `task_status_changed` PostHog event records `{ property, from, to }` with no PII. Clicking a card opens the task in the editor via the standard `onSelectNote` path.

### Table view

For `display: table`, the canvas renders [`TaskTable`](../src/components/tasks/TaskTable.tsx). Columns are driven by `view.definition.columns: string[]` and rendered through [`columnCell.tsx`](../src/components/tasks/columnCell.tsx), which maps a field name (bare key, `note.X` alias, or `file.X` derivation) to the right cell renderer — text, priority chip, date badge, status pill, chip list, or a fallback string-coerced property. Defaults to `[title, status, priority, due]` when no columns are declared. The header is sticky and click-to-sort: `getColumnSortOption` maps a column to a `SortOption`, `nextSortForColumn` flips direction when the same column is clicked twice, and the new sort serializes through `serializeSortConfig` into `onUpdateViewDefinition(filename, { sort }, rootPath?)` — the same write-back path used by `NoteList` (per [ADR 0040](../docs/adr/0040-custom-views-yml-filter-engine.md)). Inline editing is deferred; row click opens the entry in the editor on the right.

### Timeline view

For `display: timeline`, the canvas renders [`TaskTimeline`](../src/components/tasks/TaskTimeline.tsx) — a hand-rolled SVG gantt. Layout is split into a pure helper at [`src/lib/tasks/timelineLayout.ts`](../src/lib/tasks/timelineLayout.ts): `dateRangeFor` returns the visible date range with 3-day padding (or a today-centered 38-day fallback when no entries have dates), and `layoutBars` computes one `BarLayout` per dated entry plus a tally of undated entries. Swimlanes come from `groupBy.property` (default `assignee`); relationship fields use the first linked target, scalar fields use the value as-is, and entries with no value go into an `(unset)` lane. Each bar is a `<rect>` with two invisible 8px edge handles for resize and the body itself for move. Plain pointer events (no dnd-kit — the math is simpler than a list reorder); `pointerdown` reads the entry's original `start` / `due` from frontmatter so subsequent day-snapped pixel deltas convert cleanly into new ISO dates without re-parsing the bar bounds. On `pointerup` after a real drag (≥4px), one or two `onUpdateFrontmatter` calls write the new `start` and/or `due` to the task; a `task_dates_changed` PostHog event records `{ field: 'start' | 'due' | 'both' }` with no PII. A click without drag opens the task via `onSelectNote`. A footer shows the count of entries omitted because they have neither `start` nor `due`.

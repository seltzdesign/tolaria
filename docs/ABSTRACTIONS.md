...
- **`src/lib/tasks/boardColumns.ts`** — Pure helper that turns a filtered entry list plus a `ViewGroupBy` into ordered `BoardColumn`s. When the visible tasks are bound to a project that declares `statuses`, those statuses become the column order; otherwise columns are distinct field values plus an `(unset)` bucket when needed. `normalizeBoardField` strips the `note.` prefix so the writer never targets `file.X`.
- **`src/components/tasks/TaskCard.tsx`** — Shared card primitive used by the board (and reusable by the future timeline view). Renders title, priority chip, due-date badge, and assignee count for task entries; degrades to title-only for non-task entries. Click / Enter / Space invokes `onSelect`; drag handle props are spread by the board.
- **`src/components/tasks/TaskBoard.tsx`** — Kanban renderer for saved views with `display: board`. Wraps `@dnd-kit/core` `DndContext`, one `useDroppable` per `BoardColumn`, one `useDraggable` per card. `planBoardDrop` (exported, pure) resolves an active/over pair into a `{ path, field, from, to }` update; the component fires a `task_status_changed` PostHog event and calls `onUpdateFrontmatter` so the same `runFrontmatterAndApply` save path persists the new value. The unset column writes `null`.
- **`src/components/tasks/columnCell.tsx`** — Column rendering for `TaskTable`. `getColumnLabel`, `getColumnSortOption`, and `renderColumnCell` map a field name (bare key, `note.X` alias, or `file.X` derivation) to the right cell renderer: text, priority chip, date badge, status pill, chip list for relationships, numeric badge, or a fallback string-coerced property value.
- **`src/components/tasks/TaskTable.tsx`** — Table renderer for saved views with `display: table`. Columns come from `view.definition.columns` (defaults to `[title, status, priority, due]`). Rows are sorted by `view.definition.sort` via the existing `getSortComparator` from `noteListHelpers.ts`. Click a header → `nextSortForColumn` flips direction or seeds a default, and the new sort serializes through `serializeSortConfig` into `onUpdateViewDefinition(filename, { sort }, rootPath?)`. Row click opens the entry through the standard `onSelectNote` path; inline cell editing is deferred.
- **`src/lib/tasks/timelineLayout.ts`** — Pure helpers for the gantt: `dateRangeFor` (visible range with 3-day padding or today-centered fallback), `layoutBars` (one `BarLayout` per dated entry, grouped into `LaneGroup`s by `groupBy.property`; relationship fields use the first linked target), `pixelToDayOffset` and `isoDateForAbsoluteMs` / `isoDateForOffsetMs` for the drag math. Side-effect-free and exhaustively unit-tested.
- **`src/components/tasks/TaskTimeline.tsx`** — Hand-rolled SVG gantt for `display: timeline`. One `<rect>` per dated entry plus two 8px invisible edge handles for resize. Plain pointer events; `pointerdown` reads the entry's original `start` / `due` from frontmatter so subsequent day-snapped pixel deltas convert cleanly into new ISO dates. `pointerup` after a real drag writes one or two `onUpdateFrontmatter` calls and emits `task_dates_changed`; a click without drag opens the entry. A footer reports the count of entries omitted because they have neither `start` nor `due`.
